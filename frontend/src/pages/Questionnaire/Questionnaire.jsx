...
import { mapScoreToBackendEnum } from '../../utils.js/helpers';
...
const Questionnaire = () => {
  ...
  const handleOptionSelect = (value) => {
    ...
    if (current < questions.length - 1) {
      ...
      // Pass the final updated answers object directly to the save function
      saveAnswersToBackend(updatedAnswers);
    }
  };

  const saveAnswersToBackend = async (finalAnswers) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      // UPDATED: Using mapScoreToBackendEnum helper and renaming 'noise' to 'noiseTolerance'
      const lifestyleData = {
        cleanliness: mapScoreToBackendEnum('cleanliness', finalAnswers.cleanliness),
        sleep: mapScoreToBackendEnum('sleep', finalAnswers.sleep),
        noiseTolerance: mapScoreToBackendEnum('noise', finalAnswers.noise), 
        guests: mapScoreToBackendEnum('guests', finalAnswers.guests),
        sharing: mapScoreToBackendEnum('sharing', finalAnswers.sharing),
        conflict: mapScoreToBackendEnum('conflict', finalAnswers.conflict),
        pets: mapScoreToBackendEnum('pets', finalAnswers.petsAttitude)
      };
